...
inline po::variables_map helper(int argc, char** argv) 
{
    ...
    desc.add_options()
        ...
        ("keyboard", "auto-start FixStand/Velocity and use keyboard velocity commands")
        ("cmd_vel", "auto-start FixStand/Velocity and use ROS2 /cmd_vel velocity commands")
        ("no_auto_velocity", "stay in FixStand (do not auto-switch to Velocity)")
        ("cmd_vel_topic", po::value<std::string>()->default_value("/cmd_vel"), "ROS2 cmd_vel topic name")
        ("lowcmd_topic", po::value<std::string>()->default_value("rt/lowcmd"), "Unitree lowcmd DDS topic")
        ("lowstate_topic", po::value<std::string>()->default_value("rt/lowstate"), "Unitree lowstate DDS topic")
        ;
