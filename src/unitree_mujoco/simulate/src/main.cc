...
std::string ResolveDdsInterface(const std::string &interface)
{
  if (interface == "auto" || interface == "default")
  {
    return "";
  }
  return interface;
...
public:
  ...
  // length=0: spring always pulls toward anchor at z=3.
  // At pelvis z=0.793, force ≈ 200 * 2.207 ≈ 441 N upward — enough to hold
  // the robot in place while g1_ctrl initializes (~1 s).
  double length_ = 0.0;
  bool enable_ = true;
...
namespace
{
  ...
  mjModel *m = nullptr;
  mjData *d = nullptr;
  int fixstand_lock_eq_id = -1;
  int follower_fixstand_lock_eq_id = -1;

  // control noise variables
  mjtNum *ctrlnoise = nullptr;
  std::unique_ptr<ShmNavBridge> shm_nav_bridge = nullptr;
          ...
          if (sim.run)
          {
            ...
            else
            {
                     ...
                     mj::Simulate::Clock::now() - startCPU < Seconds(refreshTime))
              {
                ...
                if (fixstand_lock_eq_id >= 0)
                {
                  const bool lock_active = shm_nav_bridge ? shm_nav_bridge->FixStandLockActive() : true;
                  d->eq_active[fixstand_lock_eq_id] = lock_active ? 1 : 0;
                }
                if (follower_fixstand_lock_eq_id >= 0)
                {
                  const bool lock_active = shm_nav_bridge ? shm_nav_bridge->FixStandLockActive() : true;
                  d->eq_active[follower_fixstand_lock_eq_id] = lock_active ? 1 : 0;
                }

                // Legacy ghost follower mode. Keep disabled for real follower control.
                if (shm_nav_bridge && param::config.enable_follower_teleport == 1)
                {
                  shm_nav_bridge->TeleportFollower(m, d);
                }
...
void PhysicsThread(mj::Simulate *sim, const char *filename)
{
  ...
  if (filename != nullptr)
  {
    ...
    if (d)
    {
      ...
      mj_forward(m, d);
      const int stand_key = mj_name2id(m, mjOBJ_KEY, "stand");
      if (stand_key >= 0)
      {
        mj_resetDataKeyframe(m, d, stand_key);
      }
      else
      {
        mj_resetData(m, d);
      ...
      mj_forward(m, d);
      fixstand_lock_eq_id = mj_name2id(m, mjOBJ_EQUALITY, "fixstand_lock");
      follower_fixstand_lock_eq_id = mj_name2id(m, mjOBJ_EQUALITY, "f2_fixstand_lock");
...
void *UnitreeSdk2BridgeThread(void *arg)
{
  ...
  const std::string dds_interface = ResolveDdsInterface(param::config.interface);
  std::cout << "DDS domain/interface: " << param::config.domain_id << " / "
            << (dds_interface.empty() ? "auto" : dds_interface) << std::endl;
  unitree::robot::ChannelFactory::Instance()->Init(param::config.domain_id, dds_interface);
  shm_nav_bridge = std::make_unique<ShmNavBridge>();
  ...
  std::vector<std::unique_ptr<UnitreeSDK2BridgeBase>> interfaces;
  if (m->nu > NUM_MOTOR_IDL_GO) {
    interfaces.push_back(std::make_unique<G1Bridge>(
        m, d,
        "",
        "rt/lowcmd",
        "rt/lowstate",
        ...
        "rt/secondary_imu",
        "rt/lf/bmsstate"));
    if (mj_name2id(m, mjOBJ_ACTUATOR, "f2_left_hip_pitch") >= 0)
    {
      interfaces.push_back(std::make_unique<G1Bridge>(
          m, d,
          "f2_",
          ...
          "rt/follower/sportmodestate",
          "rt/follower/wirelesscontroller",
          "rt/follower/secondary_imu",
          "rt/follower/lf/bmsstate"));
      std::cout << "Second G1 bridge enabled for f2_ robot" << std::endl;
    }
  } else {
    interfaces.push_back(std::make_unique<Go2Bridge>(m, d));
  }
